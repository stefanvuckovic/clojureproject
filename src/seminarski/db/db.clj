...
(defn get-data 
  [col & {:keys [condition projection] :or {condition {} projection []}}]
  (mcol/find-maps db col condition projection))

(defn get-data-pagination 
  [col page limit & {:keys [condition srt fields] :or {condition {} srt {} fields []}}]
  (mquery/with-collection 
   db 
   col
...
(defn get-by-id 
  [col id & {:keys [fields] :or {fields []}}]
  (let [id (String/valueOf id)]  
    (mcol/find-map-by-id 
     db
     col 
     (Integer/parseInt id) fields)))
